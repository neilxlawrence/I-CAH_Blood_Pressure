This file plots the fits of the LMS centiles for male systolic from the file before it

```{r, note bayesian changepoint analysis is run later so we define change points manually here}
rm(list = ls())

male_systolic_change_age <- 11.5
```
...
normative_chart_male_systolic <- 
  pedbp::bp_chart("sbp", male = 1, source = "nhlbi") + 
  ylim(50,160) + 
  xlim(0,240)
normative_chart_male_systolic_data <- 
  normative_chart_male_systolic$data  
normative_chart_male_systolic_data$age_years <- 
  normative_chart_male_systolic_data$age / 12
```

```{r, split our male_lms_systolic_centiles into imputed and original lms model fits}
...
```{r, plot the imputed centiles in males with systolic values on same plot}
specific_size = 1
specific_alpha_central_lines= 0.5
male_systolic_all_imputed_fits_centile_plot <- 
  ggplot(
    data=male_lms_systolic_centiles_imputed_fits, 
    aes(x=age_to_use)) +
...
#save our plot
ggsave(filename=paste0("male_systolic_all_imputed_fits_centile_plot.tif"), 
       path=paste0("LMS_plots"),
       plot = male_systolic_all_imputed_fits_centile_plot, 
       device="tiff",  
       width=10, 
       height=5, 
       compression = "lzw", 
...
print("LMS systolic z for male under twenty are distributed:")
descr(bp_data_under_twenty_male$lms_z_systolic_imputation_0)
descr(bp_data_under_twenty_male$lms_z_systolic_imputation_1)
```

```{r, plot the complete case centiles in males and the calculated imputed centiles with systolic values on same plot}
specific_size =2
specific_alpha_central_lines=0.5
male_systolic_imputation_comparison_centile_plot <- 
  ggplot(
    data=male_lms_systolic_centiles_imputed_combined, 
    aes(x=age_to_use)) +
#  geom_line(aes(y=`0.4`))+
...
#  geom_line(data=male_lms_systolic_centiles, aes(y=`0.4`))+
  geom_line(data=male_lms_systolic_centiles_complete_case_fits, 
...
#save our plot
ggsave(filename=paste0("male_systolic_imputation_comparison_centile_plot.tif"), 
       path=paste0("LMS_plots"),
       plot = male_systolic_imputation_comparison_centile_plot, 
       device="tiff",  
       width=10, 
       height=5, 
       compression = "lzw", 
...
male_systolic_LMS_on_normative_centile_plot <- 
  ggplot(
    data=male_lms_systolic_centiles_imputed_combined, 
    aes(x=age_to_use)) +
  geom_vline(xintercept=male_systolic_change_age,
...
#put in the normative data last
    geom_line(data=subset(normative_chart_male_systolic_data, p==0.1),
            aes(x=age_years, 
                y=mmHg),
            colour="black",
            ...
            alpha=specific_alpha_peripheral_lines)+
    geom_line(data=subset(normative_chart_male_systolic_data, p==0.5),
            aes(x=age_years, 
                y=mmHg),
            colour="black",
            linewidth=specific_normative_linewidth, 
            linetype=specific_normative_linetype, 
            alpha=specific_alpha_central_lines)+
    geom_line(data=subset(normative_chart_male_systolic_data, p==0.9),
            aes(x=age_years, 
                y=mmHg),
            colour="black",
            linewidth=specific_normative_linewidth, 
...
male_systolic_LMS_on_normative_centile_plot + themepowerpointtitle

#save our plot
ggsave(filename=paste0("male_systolic_LMS_on_normative_centile_plot.tif"), 
       path=paste0("LMS_plots"),
       plot = male_systolic_LMS_on_normative_centile_plot, 
       device="tiff",  
       width=10, 
       height=5, 
       compression = "lzw", 
