This file plots the fits of the LMS centiles for fema diastolic from the file before it

```{r, note bayesian changepoint analysis is run later so we define change points manually here}
rm(list = ls())

fema_diastolic_change_age <- 7.0
```
...
normative_chart_fema_diastolic <- 
  pedbp::bp_chart("dbp", male = 0, source = "nhlbi") + ylim(50,160) + xlim(0,240)
normative_chart_fema_diastolic_data <- 
  normative_chart_fema_diastolic$data  
normative_chart_fema_diastolic_data$age_years <- 
  normative_chart_fema_diastolic_data$age / 12
```

```{r, split our fema_lms_diastolic_centiles into imputed and original lms model fits}
...
```{r, plot the imputed centiles in femas with diastolic values on same plot}
specific_size = 1
specific_alpha_central_lines= 0.5
fema_diastolic_all_imputed_fits_centile_plot <- 
  ggplot(
    data=fema_lms_diastolic_centiles_imputed_fits, 
    aes(x=age_to_use)) +
...
#save our plot
ggsave(filename=paste0("fema_diastolic_all_imputed_fits_centile_plot.tif"), 
       path=paste0("LMS_plots"),
       plot = fema_diastolic_all_imputed_fits_centile_plot, 
       device="tiff",  
       width=10, 
       height=5, 
       compression = "lzw", 
...
```{r, plot the complete case centiles in femas and the calculated imputed centiles with diastolic values on same plot}
specific_size =2
specific_alpha_central_lines=0.5
fema_diastolic_imputation_comparison_centile_plot <- 
  ggplot(
    data=fema_lms_diastolic_centiles_imputed_combined, 
    aes(x=age_to_use)) +
#  geom_line(aes(y=`0.4`))+
...
#  geom_line(data=fema_lms_diastolic_centiles, aes(y=`0.4`))+
  geom_line(data=fema_lms_diastolic_centiles_complete_case_fits, 
...
#save our plot
ggsave(filename=paste0("fema_diastolic_imputation_comparison_centile_plot.tif"), 
       path=paste0("LMS_plots"),
       plot = fema_diastolic_imputation_comparison_centile_plot, 
       device="tiff",  
       width=10, 
       height=5, 
       compression = "lzw", 
...
fema_diastolic_LMS_on_normative_centile_plot <- 
  ggplot(
    data=fema_lms_diastolic_centiles_imputed_combined, 
    aes(x=age_to_use)) +
  geom_vline(xintercept=fema_diastolic_change_age,
...
#put in the normative data last
    geom_line(data=subset(normative_chart_fema_diastolic_data, p==0.1),
            aes(x=age_years, 
                y=mmHg),
            colour="black",
            ...
            alpha=specific_alpha_peripheral_lines)+
    geom_line(data=subset(normative_chart_fema_diastolic_data, p==0.5),
            aes(x=age_years, 
                y=mmHg),
            colour="black",
            linewidth=specific_normative_linewidth, 
            linetype=specific_normative_linetype, 
            alpha=specific_alpha_central_lines)+
    geom_line(data=subset(normative_chart_fema_diastolic_data, p==0.9),
            aes(x=age_years, 
                y=mmHg),
            colour="black",
            linewidth=specific_normative_linewidth, 
...
fema_diastolic_LMS_on_normative_centile_plot +themepowerpointtitle

#save our plot
ggsave(filename=paste0("fema_diastolic_LMS_on_normative_centile_plot.tif"), 
       path=paste0("LMS_plots"),
       plot = fema_diastolic_LMS_on_normative_centile_plot, 
       device="tiff",  
       width=10, 
       height=5, 
       compression = "lzw", 
...
rm(fema_lms_diastolic_centiles)
